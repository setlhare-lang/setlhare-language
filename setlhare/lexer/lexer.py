        ...
        while i < n:
            ...
            if source.startswith("/*", i):
                ...
                if i >= n:
                    raise SetlhareSyntaxError(
                        f"unterminated block comment at {start_line}:{start_col}"
                    )
                i += 2
                col += 2
                continue
            ...
            if ch.isdigit():
                ...
                if i < n and source[i] == "." and i + 1 < n and source[i + 1].isdigit():
                    ...
                    kind = "INT"
                tokens.append(Token(kind, source[start_i:i], line, start_col))
                continue
            ...
            two = source[i : i + 2]
            if two in DOUBLE:
                tokens.append(Token(DOUBLE[two], two, line, col))
                i += 2
                col += 2
                continue
            if ch in SINGLE:
